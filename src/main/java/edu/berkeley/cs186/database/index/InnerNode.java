...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        for (int i = 0; i < this.keys.size(); i += 1) {
            if (key.compareTo(this.keys.get(i)) < 0) {
                Long pageNum = this.children.get(i);
                return BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).get(key);
            }
        }
        Long pageNum = this.children.get(this.keys.size());
        return BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).get(key);
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        return BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).getLeftmostLeaf();
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int keys_size = this.keys.size();
        Optional<Pair<DataBox, Long>> ret = Optional.empty();
        for (int i = 0; i < keys_size; i += 1) {
            if (key.compareTo(this.keys.get(i)) < 0) {
                Long pageNum = this.children.get(i);
                ret = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).put(key, rid);
                isLast = true;
                break;
            }
        }
        if (!isLast) {
            Long pageNum = this.children.get(keys_size);
            ret = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).put(key, rid);
        }
        if (!ret.isPresent()) {
            return Optional.empty();
        }
        DataBox ret_keyValue = ret.get().getFirst();
        Long  ret_pageNum = ret.get().getSecond();
        // insert sort:
        int pos = 0;
        for (; pos < this.keys.size(); pos += 1) {
            if (ret_keyValue.compareTo(this.keys.get(pos)) < 0) {
                break;
            }
        }
        this.keys.add(ret_keyValue);
        this.children.add(ret_pageNum);
        for (int i = this.keys.size() - 1; i > pos; i -= 1) {
            this.keys.set(i, this.keys.get(i - 1));
            this.children.set(i + 1, this.children.get(i));
        }
        this.keys.set(pos, ret_keyValue);
        this.children.set(pos + 1, ret_pageNum);
        if (this.keys.size() > this.metadata.getOrder() * 2) {
            List<DataBox> keysRight = new ArrayList<>(this.keys.subList(this.metadata.getOrder() + 1,
                    this.metadata.getOrder() * 2 + 1));
            List<Long> childrenRight = new ArrayList<>(this.children.subList(this.metadata.getOrder() + 1,
                    this.metadata.getOrder() * 2 + 2));
            InnerNode rightInnerNode = new InnerNode(metadata, bufferManager, keysRight, childrenRight, treeContext);
            DataBox dthValue = this.keys.get(this.metadata.getOrder());
            Long rightLeafNodePageNum = rightInnerNode.page.getPageNum();
            this.keys = new ArrayList<>(this.keys.subList(0, this.metadata.getOrder()));
            this.children = new ArrayList<>(this.children.subList(0, this.metadata.getOrder() + 1));
            Pair<DataBox, Long> returnValue = new Pair<>(dthValue, rightLeafNodePageNum);
            this.sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        while (true) {
            Long ithPageNum = this.children.get(this.children.size() - 1);
            Optional<Pair<DataBox, Long>> ret = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext,
                    ithPageNum).bulkLoad(data, fillFactor);
            if (!ret.isPresent()) {
                return Optional.empty();
            }
            DataBox ret_keyValue = ret.get().getFirst();
    ...
    public void remove(DataBox key) {
        ...
        for (int i = 0; i < this.keys.size(); i += 1) {
            if (key.compareTo(this.keys.get(i)) < 0) {
                Long pageNum = this.children.get(i);
                BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, pageNum).remove(key);
                return;
            }
        }
        Long pageNum = this.children.get(this.keys.size());
