...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode ret = this.root.get(key);
        boolean isFound = false;
        for (int i = 0; i < ret.getKeys().size(); i += 1) {
            if (ret.getKeys().get(i).compareTo(key) == 0) {
                return Optional.of(ret.getRids().get(i));
            }
        }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        Iterator<RecordId> ret = new BPlusTreeIterator(this);
        return ret;
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        Iterator<RecordId> ret = new BPlusTreeIterator(this, key);
        return ret;
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (ret.isPresent()) {
            DataBox new_key = ret.get().getFirst();
            Long leftPoint = ret.get().getSecond();
            Long oldRootPageNum = this.root.getPage().getPageNum();
            List<DataBox> newRootKeys = new ArrayList<>();
            newRootKeys.add(new_key);
            List<Long> newRootChildren = new ArrayList<>();
            newRootChildren.add(oldRootPageNum);
            newRootChildren.add(leftPoint);
            InnerNode newRoot = new InnerNode(this.metadata, this.bufferManager,
                    newRootKeys, newRootChildren, this.lockContext);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // Use the provided updateRoot() helper method to change
        // the tree's root if the old root splits.
        BPlusNode curRoot = this.root;
        while (true) {
            Optional<Pair<DataBox, Long>> ret = curRoot.bulkLoad(data, fillFactor);
            if (!ret.isPresent()) {
                this.updateRoot(curRoot);
                return;
            }
            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChild = new ArrayList<>();
            newKeys.add(ret.get().getFirst());
            newChild.add(curRoot.getPage().getPageNum());
            newChild.add(ret.get().getSecond());
            InnerNode newRoot = new InnerNode(this.metadata, this.bufferManager, newKeys, newChild, this.lockContext);
            curRoot = newRoot;
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        Optional<LeafNode> cur;
        int cur_index;
        BPlusTreeIterator(BPlusTree btree) {
            this.cur = Optional.of(btree.root.getLeftmostLeaf());
            this.cur_index = 0;
            while (this.cur.isPresent() && this.cur.get().getKeys().size() == 0) {
                this.cur = this.cur.get().getRightSibling();
            }
        }
        BPlusTreeIterator(BPlusTree btree, DataBox key) {
            this.cur = Optional.of(btree.root.get(key));
            for (int i = 0; i < this.cur.get().getRids().size(); i += 1) {
                if (this.cur.get().getKeys().get(i).compareTo(key) >= 0) {
                    this.cur_index = i;
                    return;
                }
            }
        ...
        public RecordId next() {
            // TODO(proj2): implement
            RecordId ret = cur.get().getRids().get(this.cur_index);
            this.cur_index += 1;
            if (cur_index == cur.get().getRids().size()) {
                this.cur_index = 0;
                this.cur = this.cur.get().getRightSibling();
                while (this.cur.isPresent() && this.cur.get().getKeys().size() == 0) {
                    this.cur = this.cur.get().getRightSibling();
