...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        for (; pos < this.keys.size(); pos += 1) {
            if (key.compareTo(this.keys.get(pos)) < 0) {
                break;
            }
        }
        this.keys.add(key);
        ...
        for (int i = this.keys.size() - 1; i > pos; i -= 1) {
            ...
            this.rids.set(i, this.rids.get(i - 1));
        }
        this.keys.set(pos, key);
        this.rids.set(pos, rid);
        if (this.keys.size() > this.metadata.getOrder() * 2) {
            List<DataBox> keysRight = new ArrayList<>(this.keys.subList(this.metadata.getOrder(),
                                        this.metadata.getOrder() * 2 + 1));
            List<RecordId> ridsRight = new ArrayList<>(this.rids.subList(this.metadata.getOrder(),
                                        this.metadata.getOrder() * 2 + 1));
            LeafNode rightLeafNode = new LeafNode(metadata, bufferManager, keysRight, ridsRight,
                    rightSibling, treeContext);
            DataBox dthValue = this.keys.get(this.metadata.getOrder());
            Long rightLeafNodePageNum = rightLeafNode.getPage().getPageNum();
            this.keys = new ArrayList<>(this.keys.subList(0, this.metadata.getOrder()));
            this.rids = new ArrayList<>(this.rids.subList(0, this.metadata.getOrder()));
            this.rightSibling = Optional.of(rightLeafNodePageNum);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        // TODO(proj2): implement
        int oldSize = this.rids.size();
        int maxSize = (int)(this.metadata.getOrder() * 2 * fillFactor);
        int loadSize = maxSize - oldSize;
        for (int i = 0; i < loadSize + 1; i += 1) {
            if (!data.hasNext()) {
                if (i != 0) {
                    this.sync();
                }
                ...
                Pair<DataBox, RecordId> getValue = data.next();
                if (i == loadSize) {
                    List<DataBox> keysRight = new ArrayList<>();
                    List<RecordId> ridsRight = new ArrayList<>();
                    keysRight.add(getValue.getFirst());
                    ridsRight.add(getValue.getSecond());
                    LeafNode rightLeafNode = new LeafNode(metadata, bufferManager, keysRight, ridsRight,
                            rightSibling, treeContext);
                    Pair<DataBox, Long> retVal = new Pair<>(getValue.getFirst(), rightLeafNode.page.getPageNum());
                    this.sync();
                    return Optional.of(retVal);
                } else {
                    this.keys.add(getValue.getFirst());
                    this.rids.add(getValue.getSecond());
                }
            }
        }
        return Optional.empty();
    ...
    public void remove(DataBox key) {
        ...
        for (;pos < this.keys.size(); pos += 1) {
            if (this.keys.get(pos).compareTo(key) == 0) {
                isFound = true;
                break;
            }
        }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        if (isLeftMost) {
            return new LeafNode(metadata, bufferManager, page, keys, rids, Optional.empty(), treeContext);
        }
